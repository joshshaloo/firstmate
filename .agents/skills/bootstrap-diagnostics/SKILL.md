...
description: >-
  ...
  Use whenever the session-start digest's bootstrap section prints an actionable diagnostic line - MISSING, MISSING_MANUAL, BACKEND_INVALID, NEEDS_GH_AUTH, TANGLE, STARTUP_MEMORY_BUDGET, CREW_DISPATCH invalid, FLEET_SYNC, PR_CHECK_MIGRATION, DECISION_HOLD, SECONDMATE_SYNC, SECONDMATE_LIVENESS, NUDGE_SECONDMATES, or FMX - or when a standalone bin/fm-bootstrap.sh run prints one of those lines.
...
- `MISSING: <tool> (install: <command>)` - list the missing tools to the captain with a one-line purpose each plus the printed install commands, wait for consent (one approval may cover the list), then run `bin/fm-bootstrap.sh install <approved tools...>`.
  ...
  A `quota-axi` version probe that hangs reports this same install line rather than the hung line below, so if the report survives a verified install, treat it as the hung case and investigate the binary instead of reinstalling again.
- `MISSING: <tool> (<tool> --version hung for <N>s)` - the tool is installed but its bounded version probe never answered, so this is not an install request: do not pass it to `bin/fm-bootstrap.sh install`.
  Report that the named tool is not responding, investigate why that binary hangs (a wedged process, a blocked network call, a stuck credential prompt), and rerun session start once it answers again.
...
- `DECISION_HOLD: <answer reconciliation diagnostic>` - a canonical answer file records a captain decision that is still open, records no hold identity at all, or an open captain decision already carries a resolution record without its answer file.
  Load `decision-hold-lifecycle`, inspect the named answer and decision record, then close the decision through `bin/fm-decision-hold.sh resolve` with exactly routed work rather than hand-editing the answer file or backlog.
