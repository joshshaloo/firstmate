...
- Helper scripts in `bin/` are plain bash.
  ...
  Test scripts and helpers in `tests/` are plain bash too, except a helper that must run inside a fixture's own non-bash process.
...
CI owns broad regression across required portable parallel shards, the two portable serial shards, the Herdr lane, lint, invariants, the coverage guard, and stock macOS Bash compatibility in [`.github/workflows/ci.yml`](.github/workflows/ci.yml).
...
A suite that backgrounds a real long-running process registers it with `fm_test_track_bg_pid` on the spawn line itself, before any assertion that could `fail()`, so no exit path can leave it running against a removed fixture; `fm_test_track_fixture_bg_pid` adds the independent end-of-suite process-table proof for that fixture, and `fm_test_reap_env_at_exit` covers a process only a descendant can name.
