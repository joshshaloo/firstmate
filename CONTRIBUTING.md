...
Test fixture roots under temp space must be allocated with `fm_test_tmproot` from `tests/lib.sh`, which owns the `EXIT`/`HUP`/`INT`/`TERM` traps plus the `FM_TEST_KEEP_TMP=1` debugging escape hatch; do not add per-suite temp-dir traps. Extra teardown (killing a daemon, returning a worktree, tearing down a herdr lab session) registers with `fm_test_at_exit <command>`, which runs handlers before the temp roots are removed; a handler that must fail the suite assigns `FM_TEST_EXIT_STATUS` rather than calling `exit`. `tests/fm-test-tmp-cleanup.test.sh` rejects any suite that installs its own `EXIT` trap after calling `fm_test_tmproot`.
