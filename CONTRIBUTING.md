...
Test fixture roots under temp space must be allocated with `fm_test_tmproot` from `tests/lib.sh`, which owns exit and signal cleanup plus the `FM_TEST_KEEP_TMP=1` debugging escape hatch; do not add per-suite temp-dir traps.
