...
| `/standup`         | Report what shipped in a time window, what is in progress, what is next, and what is blocked, using per-run deploy evidence rather than treating merges as shipped |
...
`/standup` defaults to 72 hours and accepts a window plus one optional project, such as `/standup 48h envy-forge`.
It separates deployed work from merged-but-not-deployed work through the local deploy-step declarations documented in [docs/configuration.md](docs/configuration.md#standup-deploy-step-declarations-configstandup-deploy-steps).
...
- [docs/verification/forge-merge-watch.md](docs/verification/forge-merge-watch.md) - maintainer verification for GitLab merge watching on arbitrary instances and for Bitbucket Cloud pull request watching and merging.
