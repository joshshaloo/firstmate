...
#     bin/fm-backlog-parse-lib.sh owns that row syntax and the full record shape;
#     this script projects it and never reparses a backlog row itself.
...
backlog_json() {  # [<backlog-path>] - defaults to this home's $BACKLOG
  fm_backlog_json "${1:-$BACKLOG}" "$SNAPSHOT_TODAY"
}
