...
Before emitting that escalation, the watcher rechecks an active no-mistakes run and treats recent run log or worktree file activity inside the wedge window as liveness, resetting the timer instead of waking on pane idleness alone.
...
A declared pause still wakes immediately through the status-signal path when the status log changes, and the stale path surfaces a no-mistakes run that reaches a parked, failed, or done state behind the old pause line.
