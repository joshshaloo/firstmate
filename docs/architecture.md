...
Those guards are proven, not assumed: spawn owns the processes, each one reports the worktree it actually entered, and the launch is refused before any `treehouse get` unless every guard is accounted for and still running, because the reset it prevents is irreversible.
A Treehouse that hands out a claimed slot anyway is still caught by the record check, which refuses that slot, leaves it, and asks for another.
...
If that recorded worktree is missing, is also claimed by another record, or still has a live or unreadable endpoint, spawn refuses instead of allocating a fresh copy.
