...
Before emitting that escalation, the watcher rechecks an active no-mistakes run step and treats the gate's own reported step activity (`bin/fm-crew-state.sh` republishes `axi status`'s `last_activity`, which no-mistakes prefixes with `quiet` once its `step_quiet_warning` window passes with no step-log or agent activity) as liveness, resetting the timer instead of waking on pane idleness alone; an absent reading is not liveness and escalates as before.
...
Those non-terminal emissions surface once and again only when the emitted line itself changes, through the same one-shot surfaced markers the watcher uses for captain-relevant statuses (`bin/fm-push-transition-lib.sh`), so a pull request sitting red costs one wake rather than one per `FM_CHECK_INTERVAL`.
The recorded bytes carry the poll's registration identity alongside the line, so re-arming a task onto a replacement pull request publishes a registration no prior marker can match and the replacement's first emission always surfaces, even when its bytes are identical.
...
A declared pause still wakes immediately through the status-signal path when the status log changes, and the stale path surfaces a no-mistakes run that reaches a parked, failed, blocked, or done state behind the old pause line exactly once per observed run-step identity - that state together with its run detail, such as which gate parked it - re-arming when that identity or the status log changes again, so a run that parks at one gate, is answered, then parks at the next still surfaces the second gate.
That authoritative recheck runs at most once per `FM_STALE_ESCALATE_SECS` per paused crew, so a pause measured in hours costs a bounded number of gate reads.
Away mode self-handles the routine pause signal and owns the later recheck.
...
Claude's `bin/fm-claude-stop-autoarm.sh` hook fires on every Stop and, when the home is eligible and still needs supervision, claims one home-scoped cycle, runs the arm wrapper as a tracked child, and translates an actionable close, or a typed failure that no live watcher explains, into one exit-2 rewake.
...
On an unmarked return, `bin/fm-afk-return.sh` owns ordered shutdown, durable catch-up evidence, and the fail-closed gate that keeps ordinary work behind every live firstmate-actionable blocker and behind any task whose status stream the keyed fold refuses to read, while still scanning every other task.
...
Each converted adapter reports its own turn lifecycle through a machine-readable contract the vendor already exposes, rather than through rendered footer text: Pi and pi-signed through the Firstmate-owned extension's agent-run and session lifecycle events, OpenCode through its plugin's semantic `session.status`, and Claude through owned `UserPromptSubmit`, `Stop`, `StopFailure`, and `SessionEnd` hooks.
The exact per-harness event set behind each contract, including how a Pi extension re-establishes state after a reload or session replacement, is owned by the [`harness-adapters` skill](../.agents/skills/harness-adapters/SKILL.md).
...
A same-id relaunch reuses the task's recorded worktree directly without asking Treehouse for another slot when the old endpoint is missing, when the old tmux endpoint is only a dead shell, or when the old Herdr endpoint is a restored shell that the Herdr backend positively proves is agent-free and then closes; [herdr-backend.md](herdr-backend.md#restart-and-liveness-behavior) owns that proof and its refusals.
If that recorded worktree is missing, is also claimed by another record, or the recorded endpoint is in any other state - live, ambiguous, unreadable, a Herdr husk that fails that proof, or on a backend with no recovery classifier at all - spawn refuses instead of allocating a fresh copy.
...
For Herdr, respawning after a server-restored layout reuses the recorded worktree and closes a same-task restored shell only after the backend proves the exact pane has no agent, no foreground job, and no child process; every unproven endpoint still refuses before any replacement endpoint is created.
