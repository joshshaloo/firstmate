...
A declared pause still wakes immediately through the status-signal path when the status log changes, and the stale path surfaces a no-mistakes run that reaches a parked, failed, or done state behind the old pause line exactly once per observed state, re-arming only when that run-step state or the status log changes again.
