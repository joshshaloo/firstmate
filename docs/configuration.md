...
`data/` holds durable private fleet records such as the project and secondmate registries, captain preferences, optional shared captain preferences, learnings, backlog, briefs, scout reports, and the canonical captain decision answers under `data/captain-decisions/`.
...
## Standup deploy step declarations (config/standup-deploy-steps)

`config/standup-deploy-steps` is an optional local, gitignored file used only by `/standup`'s shipped-work reader.
It declares which pipeline step is the production deploy for each repository, so shipped verdicts never infer a deployment from naming conventions.
Each non-empty, non-comment line is `<project><TAB><exact step name>`.
...
PR-ready metadata may also record `landing_branch=` from the forge's base branch so cleanup can verify work landed on a directed non-default branch.
...
Recording no landing target means the default branch is the target, while a recorded target that is definitively absent is refused by name rather than falling back to the default branch, so work is never judged against a branch it was not directed at.
An origin that cannot be reached or authenticated is reported as its own fact - landing unverified, naming the fetch that failed - so a transient outage is never presented as a wrongly recorded branch; all three cases refuse.
...
`fm-home-seed.sh validate` refuses any row the shared row parser cannot read, duplicate ids, duplicate homes, and nested or overlapping homes.
...
A `no-mistakes`, `tasks-axi`, or `quota-axi` probe that hits its bound reports `MISSING: <tool> (<tool> --version hung for <N>s)` instead of an install command, because reinstalling is not the remediation for a tool that is installed and not answering; tune the bounds with `FM_BOOTSTRAP_TOOL_VERSION_TIMEOUT`, `FM_TASKS_AXI_VERSION_TIMEOUT`, and `FM_QUOTA_AXI_VERSION_TIMEOUT` under "Environment variables" below.
...
FM_CLAUDE_AUTOARM_SYNC_WAIT_MS=10000   # milliseconds the --claude turn-end guard waits for the Stop auto-arm's first report for this Stop event before blocking with never-started
FM_CLAUDE_AUTOARM_DECIDE_WAIT_MS=30000   # milliseconds the --claude turn-end guard waits for a started Stop auto-arm to claim, defer, or decline before blocking with still-deciding
FM_CLAUDE_AUTOARM_EPOCH_FRESH=15   # seconds a recorded auto-arm rewake or renewal outcome counts as this event epoch's owned recovery
...
FM_CLAUDE_AUTOARM_STREAK_ESCALATE=3   # consecutive turn ends without auto-arm-owned recovery before the --claude guard escalates a PERSISTENT FAILURE
FM_CLAUDE_AUTOARM_RENEW_AFTER=   # test and diagnostic override, in seconds, for when the Stop auto-arm renews its cycle; unset derives it from the hook timeout in .claude/settings.json
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates, unless the current no-mistakes run step still reports activity (not `quiet`) in axi status; also how often a declared pause re-reads that authoritative state; stale panes whose crew is not provably working surface immediately unless they declare the pause verb
...
FM_PAUSE_RESURFACE_SECS=3600       # seconds before a declared external wait re-surfaces for a recheck in the watcher or away-mode daemon
