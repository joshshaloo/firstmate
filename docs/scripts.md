...
| `fm-standup-shipped.sh`  | Read time-windowed shipped-work evidence for `/standup`, including deploy ledgers when requested |
...
| `fm-claude-autoarm-claim-lib.sh` | Shared per-Stop-event claim handshake and verified owner liveness between the Claude Stop auto-arm and the `--claude` turn-end guard |
...
| `fm-backlog-parse-lib.sh` | Shared parser for structured backlog and done-archive rows                           |
| `fm-secondmate-registry-lib.sh` | Shared parser for registered secondmate rows                                  |
| `fm-timeout-lib.sh`      | Shared hard timeout wrapper for bounded external commands                            |
| `fm-quota-axi-lib.sh`    | Shared `quota-axi` compatibility floor for the bootstrap diagnostic                  |
| `fm-vendor-auth-probe.sh`| Run one hard-bounded, non-destructive authentication probe of a named vendor CLI and report the fact |
| `fm-jev.sh`              | Ask Jev one System One request of typed questions over one state and print the typed answers as JSON (`jev-routing` skill) |
| `fm-watch-jev.sh`        | Shadow-only Jev observer for declared-wait pause-cadence rechecks; records a private capped audit and never changes a wake |
...
| `fm-pr-check.sh`         | Record the validated PR metadata, then atomically arm a static merge poll            |
| `fm-pr-merge.sh`         | Record PR metadata, then merge a task's canonical full GitHub or Bitbucket Cloud URL |
