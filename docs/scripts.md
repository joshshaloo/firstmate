...
| `fm-standup-shipped.sh`  | Gather time-windowed shipped evidence for `/standup`, proving deploys rather than reporting merges as ships; local-only unless `--include-forge` or `--include-deploy` |
| `fm-backlog-parse-lib.sh` | Single owner of the backlog row syntax shared by `data/backlog.md` and `data/done-archive.md` |
