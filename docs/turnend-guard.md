...
An owner counts only while its pid runs this home's auto-arm script, so a recycled pid never reads as recovery, and a verified owner that has held its claim for the grace window while `state/.last-watcher-beat` has been missing or stale for the grace window is wedged rather than live.
