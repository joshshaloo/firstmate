...
The Stop auto-arm also rechecks the same identity-matched fresh-beacon predicate before translating a typed empty-cycle failure, so a cycle that started, beat, and left a live watcher after an absorbed wake closes silently instead of reporting supervision down.
