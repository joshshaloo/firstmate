...
A verified owner that has held its claim past the grace window while no healthy watcher exists is wedged; the next firing retires it and takes the claim.
...
`tests/fm-claude-stop-autoarm.test.sh` covers the auto-arm's scope, stale and live session owners, unchanged AFK and need boundaries, single-flight, the published claim and declined reasons, deferral to a verified owner, recycled-pid and wedged-owner recovery, the lifetime renewal and its settings-derived bound, a harness kill, exit-2 translation, the healthy-watcher recheck on every typed-failed close, the bounded re-arms that keep re-attaching to the surviving watcher across consecutive absorbed wakes, the default-closed alarm when a re-attach is unconfirmed or the bound is exhausted, and each alarm banner matching the health proof behind it.
