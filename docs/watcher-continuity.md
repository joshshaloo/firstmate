...
A typed empty-cycle failure also wakes the idle session unless a final identity-matched fresh-beacon check proves a watcher is already live for this home.
...
The Claude Stop auto-arm repeats that same health proof before translating the typed failure, because a concurrent absorbed-wake cycle can leave a healthy watcher even after an empty-cycle verdict was printed.
...
`tests/fm-claude-stop-autoarm.test.sh` covers the auto-arm's scope, stale and live session owners, unchanged AFK and need boundaries, single-flight, exit-2 translation, and the final healthy-watcher recheck for typed empty-cycle failures.
