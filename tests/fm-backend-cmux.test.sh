...
# Hermetic base PATH for the deliberately-missing-cmux case: a host-installed
# cmux must not satisfy it. jq is opted in so the only tool the case removes
# is cmux itself, keeping the "not found" assertion about cmux.
...
test_version_check_refuses_missing_cmux() {
  ...
  out=$( PATH="$dir/empty-fakebin:$BASE_PATH" FM_BACKEND_CMUX_BUNDLE_BIN="$dir/no-such-cmux" \
    bash -c '. "$0/bin/backends/cmux.sh"; fm_backend_cmux_version_check' "$ROOT" 2>&1 )
  status=$?
