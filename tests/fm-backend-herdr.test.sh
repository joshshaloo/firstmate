...
test_version_check_refuses_missing_herdr() {
  ...
  ln -s /usr/bin/pwd "$dir/empty-fakebin/pwd"
  # shellcheck disable=SC2016  # $0 expands inside the child Bash, like the surrounding tests.
...
test_projection_journal_v2_binds_exact_endpoint() {
  ...
  pass "herdr presentation journal: version 2 binds exact home/endpoint/parent identities atomically"
}
...
test_projection_close_rechecks_required_agent_state_at_boundary() {
  ...
  pass "herdr presentation cleanup: live agent state at the close boundary refuses mutation"
}
