...
# parsed no steps at all, a project with no declared deploy step, an ambiguous
# configured identity, a green run with no deploy step (a known fact, not a gap), a
...
# to no steps at all, which is the separate "could not be checked" case.
make_fakebin() {  # <dir>
  local fb
  fb=$(fm_fakebin "$1")
  cat > "$fb/gh" <<'SH'
...
set_deploy_step() {  # <home> <project> <exact step name>
  printf '%s\t%s\n' "$2" "$3" >> "$1/config/standup-deploy-steps"
}
...
# --- the evidence ledger owns deploy-step identity and verdicts -------------
#
# One owner answers "which step is the production deploy": the local declaration
# in config/standup-deploy-steps. A differently named step can still ship when it
# is declared for that repository, while a missing declaration leaves the run
# unrecognized instead of turning into a firm negative.

: > "$HOME_A/config/standup-deploy-steps"
set_deploy_step "$HOME_A" alpha "Ship it"
declared=$(jq -nc '{steps:[
  {uuid:"{step-build}", name:"Build and test",
   state:{name:"COMPLETED", result:{name:"SUCCESSFUL"}}},
  {uuid:"{step-ship}", name:"Ship it", deployment_environment:{name:"production"},
   state:{name:"COMPLETED", result:{name:"SUCCESSFUL"}}}]}')
json=$(FAKE_BKT_RUNS="$runs" FAKE_BKT_STEPS="$declared"   FAKE_BKT_LOG="$(recorded_log "$head_shipped")"   run "$HOME_A" "$FB_A" --window 72h --include-deploy --json)
v11=$(printf '%s' "$json" | jq -r '.merges[] | select(.title | test("#11")) | .deployed')
[ "$v11" = yes ] || fail "a declared deploy step identifies its own step, got: $v11"
assert_contains "$(printf '%s' "$json" | jq -r '.deploys[].evidence_found')" "declared production deploy step 'Ship it'"   "a configured step is reported in the evidence ledger"
assert_contains "$(printf '%s' "$json" | jq -r '.deploys[].evidence_found')" "recorded production head"   "the ledger records the production head evidence"
assert_contains "$(printf '%s' "$json" | jq -r '.deploys[].evidence_sought')" "declared production deploy step"   "the ledger says what evidence was sought"
toon=$(FAKE_BKT_RUNS="$runs" FAKE_BKT_STEPS="$declared"   FAKE_BKT_LOG="$(recorded_log "$head_shipped")"   run "$HOME_A" "$FB_A" --window 72h --include-deploy)
assert_contains "$toon" "evidence_sought" "TOON shows the evidence ledger per run"
assert_not_contains "$toon" "declared configuration/" "TOON does not expose the internal identity labels"

# A second repository can declare a different production deploy step name without
# teaching the reader a new naming convention.
HOME_B=$(make_home home-b)
FB_B=$(make_fakebin "$HOME_B")
printf -- '- alpha [no-mistakes +yolo] - Fixture project (cloned 2026-08-01)
- beta [no-mistakes] - Second fixture project (cloned 2026-08-01)
' > "$HOME_B/data/projects.md"
make_project "$HOME_B" alpha "git@bitbucket.org:acme/alpha.git" >/dev/null
REPO_B_BETA=$(make_project "$HOME_B" beta "git@bitbucket.org:acme/beta.git")
set_deploy_step "$HOME_B" alpha "Deploy to production"
set_deploy_step "$HOME_B" beta "Release to production"
beta_head=$(git -C "$REPO_B_BETA" rev-parse main~1)
beta_runs=$(jq -nc --arg ok "$beta_head" '
  {values:[{build_number:2041, state:{result:{name:"SUCCESSFUL"}},
            target:{ref_name:"main", commit:{hash:$ok}}, created_on:"2026-08-26T10:00:00Z"}]}')
beta_steps=$(jq -nc '{steps:[{uuid:"{step-release}", name:"Release to production",
                              state:{name:"COMPLETED", result:{name:"SUCCESSFUL"}}}]}')
json=$(FAKE_BKT_RUNS="$beta_runs" FAKE_BKT_STEPS="$beta_steps"   FAKE_BKT_LOG="$(recorded_log "$beta_head")"   run "$HOME_B" "$FB_B" --window 72h --project beta --include-deploy --json)
v_beta=$(printf '%s' "$json" | jq -r '.merges[] | select(.title | test("#11")) | .deployed')
[ "$v_beta" = yes ] || fail "a second repository's differently named deploy step must ship, got: $v_beta"
assert_contains "$(printf '%s' "$json" | jq -r '.projects[] | select(.id == "beta") | .deploy_step')" "Release to production"   "the project row carries the repository's declared production deploy step"

# With no per-repository declaration, the step is not recognized here. That is an
# unknown, not a firm no. With a declaration present and absent from a parsed run,
# the run is known not to contain the production deploy step, so the firm negative
# remains reachable.
: > "$HOME_A/config/standup-deploy-steps"
unmatched=$(jq -nc '{steps:[{uuid:"{step-release}", name:"Release to production",
                             state:{name:"COMPLETED", result:{name:"SUCCESSFUL"}}}]}')
json=$(FAKE_BKT_RUNS="$runs" FAKE_BKT_STEPS="$unmatched"   FAKE_BKT_LOG="$(recorded_log "$head_shipped")"   run "$HOME_A" "$FB_A" --window 72h --include-deploy --json)
deployed=$(printf '%s' "$json" | jq -r '[.merges[].deployed] | unique | join(",")')
[ "$deployed" = unknown ]   || fail "an undeclared project must leave deploy verdicts unknown, got: $deployed"
assert_contains "$(printf '%s' "$json" | jq -r '.deploys[].evidence_unavailable')" "no declared production deploy step"   "the ledger names the missing declaration instead of pretending no deploy ran"

: > "$HOME_A/config/standup-deploy-steps"
set_deploy_step "$HOME_A" alpha "Deploy to production"
json=$(FAKE_BKT_RUNS="$runs" FAKE_BKT_STEPS="$unmatched"   FAKE_BKT_LOG="$(recorded_log "$head_shipped")"   run "$HOME_A" "$FB_A" --window 72h --include-deploy --json)
deployed=$(printf '%s' "$json" | jq -r '[.merges[].deployed] | unique | join(",")')
[ "$deployed" = no ]   || fail "a parsed run without the declared production deploy step must leave a firm negative reachable, got: $deployed"
assert_contains "$(printf '%s' "$json" | jq -r '.deploys[].counted')" "declared deploy step is not in this run"   "the row distinguishes an absent declared step from an unrecognized step"
assert_contains "$(printf '%s' "$json" | jq -r '.deploys[].evidence_unavailable')" "not present in this run"   "the ledger records the absent declared step as the reason"

# Two steps with the same declared name are not a convention fallback; they are a
# non-unique declaration match, so the verdict remains unknown and the ledger says
# what prevented the production step from being selected.
ambiguous=$(jq -nc '{steps:[
  {uuid:"{step-prod-1}", name:"Deploy to production",
   state:{name:"COMPLETED", result:{name:"SUCCESSFUL"}}},
  {uuid:"{step-prod-2}", name:"Deploy to production",
   state:{name:"COMPLETED", result:{name:"SUCCESSFUL"}}}]}')
json=$(FAKE_BKT_RUNS="$runs" FAKE_BKT_STEPS="$ambiguous"   FAKE_BKT_LOG_STEP_stepprod1="$(recorded_log "$head_shipped")"   run "$HOME_A" "$FB_A" --window 72h --include-deploy --json)
assert_contains "$(printf '%s' "$json" | jq -r '.deploys[].counted')" "appears more than once"   "several declared-name matches are reported as ambiguous"
assert_contains "$(printf '%s' "$json" | jq -r '.deploys[].evidence_unavailable')" "not unique"   "an ambiguous declaration match is disclosed in captain-facing nouns"
v11=$(printf '%s' "$json" | jq -r '.merges[] | select(.title | test("#11")) | .deployed')
...
# and the payload parsed steps of which none is the declared deploy step. Only the
# last is a known absence; the other two are gaps.

json=$(FAKE_BKT_RUNS="$runs" FAKE_BKT_VIEW_FAIL=1041   run "$HOME_A" "$FB_A" --window 72h --include-deploy --json)
deployed=$(printf '%s' "$json" | jq -r '[.merges[].deployed] | unique | join(",")')
[ "$deployed" = unknown ] || fail "unreadable steps must leave merges unknown, got: $deployed"
assert_contains "$(printf '%s' "$json" | jq -r '.deploys[].counted')" "steps could not be read"   "a run whose steps could not be read says so"

json=$(FAKE_BKT_RUNS="$runs" FAKE_BKT_STEPS='{"steps":[]}'   run "$HOME_A" "$FB_A" --window 72h --include-deploy --json)
deployed=$(printf '%s' "$json" | jq -r '[.merges[].deployed] | unique | join(",")')
[ "$deployed" = unknown ] || fail "an unparseable step payload must leave merges unknown, got: $deployed"
assert_contains "$(printf '%s' "$json" | jq -r '.deploys[].counted')" "did not parse"   "a payload that yielded no steps is a gap, not a definitive absence"
assert_contains "$(printf '%s' "$json" | jq -r '.omitted[].surface')" "neither be found nor ruled out"   "a payload that parsed nothing is disclosed as its own fact"

nodeploy=$(jq -nc '{steps:[{uuid:"{s1}", name:"Build and test",
                            state:{name:"COMPLETED", result:{name:"SUCCESSFUL"}}}]}')
json=$(FAKE_BKT_RUNS="$runs" FAKE_BKT_STEPS="$nodeploy"   run "$HOME_A" "$FB_A" --window 72h --include-deploy --json)
deployed=$(printf '%s' "$json" | jq -r '[.merges[].deployed] | unique | join(",")')
[ "$deployed" = no ]   || fail "a green run without the declared deploy step must be a firm negative, got: $deployed"
assert_contains "$(printf '%s' "$json" | jq -r '.deploys[].counted')" "declared deploy step is not in this run"   "a run with no declared deploy step says so"
assert_contains "$(printf '%s' "$json" | jq -r '.omitted[].surface')" "no run recorded a production head"   "a repository with no deploy evidence is disclosed, not read as a deploy train"
...
json=$(FAKE_BKT_RUNS="$runs_mixed"   FAKE_BKT_STEPS_1402="$nodeploy"   FAKE_BKT_STEPS_1401="$(steps_json COMPLETED SUCCESSFUL)"   FAKE_BKT_LOG="$(recorded_log "$head_shipped")"   run "$HOME_A" "$FB_A" --window 72h --include-deploy --json)
v11=$(printf '%s' "$json" | jq -r '.merges[] | select(.title | test("#11")) | .deployed')
v12=$(printf '%s' "$json" | jq -r '.merges[] | select(.title | test("#12")) | .deployed')
[ "$v11" = yes ] || fail "the deploy run still proves its own containment, got: $v11"
[ "$v12" = no ]   || fail "a build-only run is a known fact, not a gap, so a firm negative stays reachable, got: $v12"
assert_not_contains "$(printf '%s' "$json" | jq -r '.omitted[].surface')" "deploy read was partial"   "a build-only run must not manufacture a partial-read disclosure"

pass "a parsed run with no declared deploy step is a known fact; an unread or unparsed one is a gap"
