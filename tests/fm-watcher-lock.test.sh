...
test_singleton_start() {
  ...
  pid1=$!
  fm_test_track_fixture_bg_pid "$pid1" "FM_STATE_OVERRIDE=$state"
  ...
  pid2=$!
  fm_test_track_fixture_bg_pid "$pid2" "FM_STATE_OVERRIDE=$state"
  i=0
  while [ "$i" -lt 50 ]; do
    live=0
...
test_watch_restart_rejects_reused_pid() {
  ...
  pid=$!
  fm_test_track_fixture_bg_pid "$pid" "FM_HOME=$dir"
...
test_watch_restart_attaches_to_healthy_peer() {
  ...
  mark_pr_check_migration_complete "$state"
  start_settled_peer node -e 'process.on("SIGTERM", () => {}); require("fs").writeFileSync(process.argv[1], "ready\n"); setTimeout(() => {}, 300000)' "$ready" \
    || fail "peer node process never reached its own image"
  peer=$PEER_PID
  # Reaching the node image does not mean the SIGTERM handler is installed yet;
  # a restart TERM in that window kills the peer and the fresh child rightly
  # reclaims its dead lock. The peer publishes readiness only after the handler.
  i=0
  while [ "$i" -lt 200 ] && [ ! -e "$ready" ]; do
    sleep 0.05
    i=$((i + 1))
  done
  [ -e "$ready" ] || fail "peer node process never installed its SIGTERM handler"
  ...
  armpid=$!
  fm_test_track_fixture_bg_pid "$armpid" "FM_HOME=$dir"
  i=0
  while [ "$i" -lt 80 ]; do
...
test_arm_self_eviction_is_loud_without_successor() {
  ...
  armpid=$!
  fm_test_track_fixture_bg_pid "$armpid" "FM_STATE_OVERRIDE=$state"
  i=0
  while [ "$i" -lt 80 ]; do
...
test_arm_attaches_and_waits_for_live_fresh_watcher() {
  ...
  PATH="$fakebin:$PATH" FM_STATE_OVERRIDE="$state" FM_POLL=5 FM_SIGNAL_GRACE=1 FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  wpid=$!
  fm_test_track_fixture_bg_pid "$wpid" "FM_STATE_OVERRIDE=$state"
  i=0
  while [ "$i" -lt 60 ]; do
...
SH
  ...
  first_arm=$!
  fm_test_track_fixture_bg_pid "$first_arm" "FM_STATE_OVERRIDE=$state"
  ...
  successor_arm=$!
  fm_test_track_fixture_bg_pid "$successor_arm" "FM_STATE_OVERRIDE=$state"
  i=0
  while [ "$i" -lt 80 ]; do
    grep -qF 'watcher: started pid=' "$armout" 2>/dev/null && break
    sleep 0.1
  ...
  fm_test_track_bg_pid "$successor_pid"
